...
export async function rewardPvpResultForHuman(matchId,humanId){const economy=await getSetting('economy')||{};return withTransaction(async client=>{const row=(await client.query("SELECT * FROM matches WHERE id=$1 AND status='finished' FOR UPDATE",[matchId])).rows[0];if(!row)return null;const draw=!row.winner_id,win=row.winner_id===humanId;const xp=draw?(economy.pvpDrawXp??75):win?(economy.pvpWinXp??120):(economy.pvpLoseXp??60),points=draw?(economy.pvpDrawPoints??18):win?(economy.pvpWinPoints??35):(economy.pvpLosePoints??10);const inserted=await insertReward(client,humanId,'pvp',matchId,xp,points,{gameKey:row.game_key,result:draw?'draw':win?'win':'loss',countGame:true});return [{userId:humanId,xp:inserted?xp:0,points:inserted?points:0}];});}
